...
from bot_manager import init_bot_manager, managed_bots
from web import setup_web_module, create_web_app
from datetime import datetime

WEB_PORT = int(__import__('os').getenv('WEB_PORT', '3000'))
...
def get_ssh_session(server_id, ip, username, key_content):
    ...
    try:
        ...
        # Try different key types (DSSKey removed for compatibility)
...
async def on_startup(_):
    ...
    # Start web dashboard
    try:
        setup_web_module(bot, servers_collection, active_sessions, managed_bots)
        web_app = create_web_app()
        runner = aio_web.AppRunner(web_app)
        await runner.setup()
        site = aio_web.TCPSite(runner, '0.0.0.0', WEB_PORT)
        await site.start()
        logger.info(f"✅ Web dashboard running on http://0.0.0.0:{WEB_PORT}")
    except Exception as e:
...
async def view_server(callback: types.CallbackQuery):
    ...
    try:
        ...
        kb.add(
            ...
            InlineKeyboardButton("💻 Terminal", callback_data=f"terminal_{server_id}")
        )
        kb.add(
...
@dp.callback_query_handler(lambda c: c.data.startswith("terminal_"))
async def enter_terminal(callback: types.CallbackQuery):
    """Activate terminal mode for a server"""
    server_id = callback.data.split('_')[1]
    
    # Set user state to terminal mode
    user_input[callback.from_user.id] = {'action': 'terminal', 'server_id': server_id}
    
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("❌ Exit Terminal", callback_data=f"exit_term_{server_id}"))
    
    await callback.message.edit_text(
        "💻 <b>Terminal Mode Activated</b>\n\n"
        "Send any Linux command here. It will be executed directly on the server.\n\n"
        "<i>Click the button below or type <code>exit</code> to leave.</i>",
        parse_mode="HTML",
        reply_markup=kb
    )

@dp.callback_query_handler(lambda c: c.data.startswith("exit_term_"))
async def exit_terminal(callback: types.CallbackQuery):
    """Exit terminal mode and return to server menu"""
    server_id = callback.data.split('_')[2]
    
    # Clear user state
    user_input.pop(callback.from_user.id, None)
    
    # Trick the bot into reloading the server menu
    callback.data = f"server_{server_id}"
    await view_server(callback)

@dp.message_handler(lambda message: message.from_user.id in user_input and user_input[message.from_user.id].get('action') == 'terminal')
async def handle_terminal_command(message: types.Message):
    ...
    if command.lower() in ['exit', 'quit', 'cancel']:
        user_input.pop(uid, None)
        await message.answer("🚪 <b>Exited terminal mode.</b>", parse_mode="HTML")
    ...
    if not ssh:
        await message.answer("❌ SSH session not active. Please go to Server Settings and Reconnect.")
        return

    try:
        # Send processing message
        processing_msg = await message.answer("⏳ <i>Executing...</i>", parse_mode="HTML")
        ...
        # Combine output or use fallback text
        ...
        # Escape HTML to prevent broken formatting
        safe_response = html.escape(response)
        
        # Edit the processing message with the result
        kb = InlineKeyboardMarkup().add(InlineKeyboardButton("❌ Exit Terminal", callback_data=f"exit_term_{server_id}"))
        await processing_msg.edit_text(
            f"<b>$</b> <code>{html.escape(command)}</code>\n\n"
            f"<pre><code>{safe_response}</code></pre>",
            parse_mode="HTML",
            reply_markup=kb
        )
